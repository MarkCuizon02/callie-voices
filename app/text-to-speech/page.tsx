...
export default function Home() {
  ...
  const handleAutoGenerateText = async () => {
    ...
    if (selectedModel === 'elevenlabs') {
      voice = elevenLabsVoices.find(v => v.id === selectedVoice);
          ...
          <SelectTrigger className="w-64">
            <div className="flex items-center gap-2 w-full">
              <span className="flex items-center justify-center h-5 w-5">
                {(() => {
                  const v = elevenLabsVoices.find(v => v.id === selectedVoice);
              ...
              <span className="truncate max-w-full">
                {isLoadingVoices
                  ? "Loading voices..."
                  : elevenLabsVoices.find(v => v.id === selectedVoice)
                    ? `${elevenLabsVoices.find(v => v.id === selectedVoice)?.name} - ${elevenLabsVoices.find(v => v.id === selectedVoice)?.description || elevenLabsVoices.find(v => v.id === selectedVoice)?.category || ""}`
                    : "Select a voice"}
              </span>
            </div>
          </SelectTrigger>
  ...
  useEffect(() => {
    if (selectedModel === "openai") {
      ...
      const voice = elevenLabsVoices.find(v => v.id === selectedVoice);
      if (voice) {
        setModelInfo({
          name: voice.name,
